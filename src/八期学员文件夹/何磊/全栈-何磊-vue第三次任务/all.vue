<template>
    <div>
        <music>
            <!-- 热榜歌手 -->
            <template v-slot:hot-artists>
                <div class="artists-list">
                    <div v-for="(artist,index) in hotArtists" :key="index" class="artist">
                        <img :src="artist.picUrl" alt="" class="singer">
            ...
            <!-- 新歌榜 -->
            <template v-slot:newSongs>
                <div class="head">
                    <div>
                        <h2>{{ this.newChart.name }}</h2>
                        <span>{{ this.newChart.description }}</span>
                    </div>
                    <img :src="this.newChart.coverImgUrl" alt="">
                </div>
                <hr>
                <h3 v-for="(newSong,index) in newSongs" :key="index" class="ranking">
                    <span>{{ index+1 }}.</span>
                    <span>{{ newSong.name }}</span>
                </h3>
            </template>
            <!-- 热歌榜单 -->
            <template v-slot:hotSongs>
                <div class="head">
                    <div>
                        <h2>{{ this.hotChart.name }}</h2>
                        <span>{{ this.hotChart.description }}</span>
                    </div>
                    <img :src="this.hotChart.coverImgUrl" alt="">
                </div>
                <hr>
                <h3 v-for="(hotSong, index) in hotSongs" :key="index" class="ranking">
                    <span>{{ index + 1 }}.</span>
                    <span>{{ hotSong.name }}</span>
                </h3>
            </template>


            <!-- 原创榜  -->
            <template v-slot:originSongs>
                <div class="head">
                    <div>
                        <h2>{{ this.originChart.name }}</h2>
                        <span>{{ this.originChart.description }}</span>
                    </div>
                    <img :src="this.originChart.coverImgUrl" alt="">

                </div>
                <hr>
                <h3 v-for="(originSong, index) in originSongs" :key="index" class="ranking">
                    <span>{{ index + 1 }}.</span>
                    <span>{{ originSong.name }}</span>
                </h3>
            </template>
