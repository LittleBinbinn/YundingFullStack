<template>
    <div class="app">
        <music>
            <template #hotSinger>
                <div class="people">
                    <div class="one" v-for="(item, index) in hotSinger" :key="index">
                        <img :src="hotSinger[index].altUrl" alt=""><br>
                        <span> {{ hotSinger[index].name }}</span>
                    </div>
                </div>
            </template>
            <template #introduce>
                <div class="song" v-for="(item,index) in introduce " :key="index">
                    <div class="sing">
                        <h3>{{ introduce[index].name }}</h3>
                        <span style="font-size: 13px;">{{ introduce[index].description }}</span>
                    </div>
                    <img :src="introduce[index].coverImgUrl" id="list" alt="">
                </div>
            </template>
            <template #musicList>
                <div class="musicList">
                    <div class="first">
                        <ul>
                            <li v-for="(item, index) in hotSongList" :key="item">
                                {{ index + 1 + "-" + item.name }}
                            </li>
                        </ul>
                    </div>
                    <div class="first">
                        <ul>
                            <li v-for="(item, index) in newSongList" :key="item">
                                {{ index + 1 + "-" + item.name }}
                            </li>
                        </ul>
                    </div>
                    <div class="first">
                        <ul>
                            <li v-for="(item, index) in originalSongList" :key="item">
                                {{ index + 1 + "-" + item.name }}
                            </li>
                        </ul>
                    </div>
